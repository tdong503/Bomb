...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on('room:joined', (r, player) => { setRoom(r); setYou(player); });
  ...
  const startGame = useCallback(() => {
    if (!room) return; getSocket().emit('room:start');
  }, [room]);

  const drawCard = useCallback(() => { getSocket().emit('game:draw'); }, []);
  const playCard = useCallback((cardId: string) => { getSocket().emit('game:playCard', { cardId }); }, []);
  ...
  return (
    <div className="app-shell">
      <header className="app-header">
        <h1 className="gradient-text">Bomb Card Game</h1>
        <div className="status">Conn: {connected ? '✅' : '❌'} {room && <span>Room {room.roomId}</span>}</div>
      </header>
      {error && <div className="error-banner">{error}</div>}
      {!room && <Lobby onCreate={createRoom} onJoin={joinRoom} />}
      {room && <RoomView room={room} you={you} hand={hand} onStart={startGame} onDraw={drawCard} onPlay={playCard} />}
      {seeFuture && (
      ...
      {defuseBomb && room && (
        <div className="overlay">
          <div className="overlay-content">
            ...
            <input type="range" min={0} max={room.deckCount} value={defusePos} onChange={e=>setDefusePos(Number(e.target.value))} />
            ...
            <div className="actions-row wrap">
              ...
              <button onClick={()=>setDefusePos(Math.floor(Math.random()*(room.deckCount+1)))}>随机</button>
            </div>
            <div className="actions-row">
      ...
      <footer className="app-footer">Prototype build – features incomplete</footer>
    </div>
  );
};
